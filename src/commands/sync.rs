...
pub fn run(ctx: &mut Context, args: Args) -> i32 {
    ...
    if args.target.is_dir() {
        ...
        ctx.out.info(&format!("Found {} under {}.", parts.join(" + "), args.target.display()), None);
        preset_files = files;
    ...
    for f in &preset_files {
        let text = match std::fs::read_to_string(f) {
            Ok(t) => t,
            Err(e) => {
                ctx.out.error(&format!("{}: cannot read: {e}", f.display()), None);
                had_error = true;
                if args.strict {
                    return exit_codes::INPUT_FILE_ERROR;
                }
        ...
        let raw: Value = match yaml_from_str(&text) {
            Ok(v) => v,
            Err(e) => {
                ctx.out.error(&format!("{}: not valid YAML: {e}", f.display()), None);
                had_error = true;
                if args.strict {
                    return exit_codes::INPUT_FILE_ERROR;
                }
        ...
        if !filter_blocking(&sem).is_empty() {
            for i in &sem {
                if i.severity == Severity::Error {
                    ctx.out.error(&format!("{}: {}: {}", f.display(), i.path, i.message), None);
                }
            }
            had_error = true;
            if args.strict {
        ...
        for i in &sem {
            if i.severity != Severity::Error {
                ctx.out.warn(&format!("{}: {}: {}", f.display(), i.path, i.message), None);
            }
        }
    ...
    let controller_encoded: Option<(PathBuf, Vec<u8>)> = if let Some(p) = &controller_file {
        match load_controller_yaml(p).and_then(|c| {
            ...
            Err(e) => {
                ctx.out.error(&format!("Could not encode {}: {e}", p.display()), None);
                had_error = true;
                if args.strict {
    ...
    for (f, preset) in loaded {
        let (bank_msg, preset_msg) = match encode_navigate_to(preset.bank, preset.number) {
            Ok(t) => t,
            Err(e) => {
                ctx.out.error(&format!("Could not encode {}: {e}", f.display()), None);
                if args.strict {
                    return exit_codes::ENCODE_ERROR;
                }
                continue;
    ...
    if args.dry_run {
        ...
        ctx.out.emit_result(
            ...
            Some(&format!("Dry run: {item_count} item(s), {total_bytes} bytes total.")),
        );
    ...
    for (idx, (f, preset, bank_msg, preset_msg, save_msg)) in encoded.iter().enumerate() {
        ...
        helpers::drain_settle(
            ...
            if need_bank_change { Duration::from_secs(4) } else { Duration::from_millis(1500) },
        );
        ...
        if acked {
            ...
            failures += 1;
            ctx.out.warn(&format!("  fail {label} -- no acknowledgement"), None);
            results.push(json!({
                "path": f.display().to_string(),
                "bank": preset.bank + 1,
                "number": preset.number,
