...
pub fn encode_simple_preset(preset: &Preset, save_to_current: bool) -> Result<Vec<u8>, EncodeError> {
...
pub fn encode_controller_with(
    ...
    debug_assert!(
        connectors_by_slug.iter().map(|(s, _)| *s).eq(CONNECTOR_SLUGS.iter().copied())
    );
    ...
    segments.push(encode_segment(
        ...
        &[if controller.loop_bypass_persistent { 0x7F } else { 0 }],
    ));
...
pub fn encode_select_preset_with(device: DeviceProfile, preset: u8) -> Result<Vec<u8>, EncodeError> {
