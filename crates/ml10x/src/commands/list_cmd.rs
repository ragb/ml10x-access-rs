...
pub fn run(ctx: &mut Context, args: Args) -> i32 {
    ...
    if from_device && args.dir_arg.is_some() {
        ctx.out
            .error("Pass either a DIR or --device, not both.", None);
...
fn run_directory(
    ...
    for f in files {
        let preset = match load_preset_yaml(&f) {
            ...
            Err(e) => {
                ctx.out
                    .warn(&format!("  could not load {}: {e}", f.display()), None);
                continue;
            }
        };
    ...
    if rows.is_empty() {
        let suffix = only_bank
            .map(|b| format!(" (bank {b})"))
            .unwrap_or_default();
        ctx.out.info(
            &format!("No presets found in {}{}.", dir.display(), suffix),
            None,
        );
        ctx.out
            .emit_result(&json!({"ok": true, "presets": []}), None);
        return exit_codes::OK;
    }

    ctx.out.info(
        &format!("{} preset(s) in {}:", rows.len(), dir.display()),
        None,
    );
    ctx.out.info(
        &format!("  {:<5}{:<5}{:<10}NAME", "BANK", "NUM", "MODE"),
        None,
    );
    for r in &rows {
        ...
        ctx.out
            .info(&format!("  {bank:<5}{num:<5}{mode:<10}{name}"), None);
    }
    ctx.out
        .emit_result(&json!({"ok": true, "presets": rows}), None);
    exit_codes::OK
}
...
fn run_device(ctx: &mut Context, only_bank: Option<u8>, include_empty: bool) -> i32 {
    ...
    for &bank in &banks_to_query {
        ...
        for preset_num in 0u8..128 {
            if let Some(p) =
                helpers::try_read_preset(&mut io, bank, preset_num, Duration::from_millis(1500))
            {
        ...
        ctx.out.detail(
            &format!(
                "Bank {}: {} preset(s) read.",
                bank + 1,
                all_names.iter().filter(|(b, _, _)| *b == bank).count()
            ),
            None,
        );
    }
    ...
    if ctx.out.json_mode {
        ctx.out.emit_result(
            &json!({"ok": true, "source": "device", "presets": rows}),
            None,
        );
        return exit_codes::OK;
    }
    ...
    if rows.is_empty() {
        let suffix = only_bank
            .map(|b| format!(" in bank {b}"))
            .unwrap_or_default();
        ctx.out.info(
            &format!(
                "No named presets on the device{suffix} (use --empty to include 'Empty' slots)."
            ),
            None,
        );
        ctx.out
    ...
    ctx.out.info(
        &format!("{} named preset(s) on the device:", rows.len()),
        None,
    );
    ctx.out
        .info(&format!("  {:<5}{:<5}NAME", "BANK", "NUM"), None);
    for r in &rows {
        let bank = r["bank"].as_u64().unwrap_or(0);
        let num = r["number"].as_u64().unwrap_or(0);
        let name = r["name"].as_str().unwrap_or("");
