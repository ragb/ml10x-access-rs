...
pub fn run(ctx: &mut Context, args: Args) -> i32 {
    ...
    if let Err(e) = std::fs::create_dir_all(&args.out_dir) {
        ctx.out.error(
            &format!("Could not create {}: {e}", args.out_dir.display()),
            None,
        );
    ...
    ctx.out
        .info(&format!("Opening MIDI ports matching {port:?}."), None);
    ...
    while Instant::now() < deadline {
        ...
        let _ = parse_header_with(crate::device::ML10X, &msg, true)
            ...
            .map_err(|e| {
                ctx.out
                    .warn(&format!("  malformed SysEx kept in raw capture: {e}"), None);
            });
        received.push(msg);
    }
    ...
    let _ = std::fs::write(
        &raw_file,
        serde_json::to_string_pretty(&raw_doc).unwrap_or_default(),
    );
    ctx.out.detail(
        &format!(
            "Saved {} raw inbound messages to {}.",
            received.len(),
            raw_file.display()
        ),
        None,
    ...
    // Pre-extract the UUID from the controller-info message (the controller
    // settings message doesn't carry it — it's in a separate p1=17 frame).
    let uuid = helpers::extract_uuid(&received);

    for msg in &received {
        match classify_inbound(msg) {
            Ok(InboundMessage::Preset {
                bank,
                number,
                preset,
            }) => {
                let path = args
                    .out_dir
                    .join(format!("bank-{}", bank + 1))
                    .join(format!("preset-{:03}.yaml", number));
                if let Err(e) = dump_preset_yaml(&preset, &path) {
                    ctx.out
                        .warn(&format!("  could not write {}: {e}", path.display()), None);
                    continue;
                }
                let rel = path
                ...
                ctx.out
                    .detail(&format!("  wrote {} ({:?}).", rel, preset.name), None);
            }
            Ok(InboundMessage::Controller(mut ctrl)) => {
                if let Some(u) = &uuid {
                    ctrl.uuid = u.clone();
                }
                ...
                ctx.out.detail(
                    &format!(
                        "  wrote {}.",
                        path.strip_prefix(&args.out_dir).unwrap_or(&path).display()
                    ),
                    None,
                );
            }
            Ok(_) => {} // PresetNames / ControllerInfo / EditorEvent / Other — no YAML to write
            Err(e) => {
                ctx.out
                    .warn(&format!("  skipping malformed inbound: {e}"), None);
            }
        }
    }
    ...
    if args.all_presets {
        ctx.out.info(
            "Walking all 4 banks x 128 presets. Takes about 2 minutes.",
            None,
        );
        ...
        let mut io = match MidiIo::open(&port) {
            ...
            Err(e) => {
                ctx.out.error(
                    &format!("Could not reopen the device for full walk: {e}"),
                    None,
                );
                return exit_codes::DEVICE_UNAVAILABLE;
            }
            ...
            for preset_num in 0u8..128 {
                ...
                for attempt in 0..2 {
                    ...
                    while Instant::now() < deadline {
                        ...
                        {
                            // Trust the bank/number we asked for, not the
                            // wire bytes — the device occasionally echoes
                            // stale slot numbers under repeated selects.
                            let path = args
                                .out_dir
                                .join(format!("bank-{}", bank + 1))
                                .join(format!("preset-{:03}.yaml", preset_num));
                            let mut p = preset;
                            p.bank = bank;
                            p.number = preset_num;
