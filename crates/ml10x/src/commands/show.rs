...
use ml10x_core::yaml::preset_to_yaml_string;
...
pub fn run(ctx: &mut Context, args: Args) -> i32 {
    ...
    let blocking = all_issues
        .iter()
        .any(|i| i.severity == Severity::Error || (args.strict && i.severity == Severity::Warning));
    ...
    if let Some(p) = maybe_preset {
        match preset_to_yaml_string(&p) {
            Ok(s) => print!("{s}"),
            Err(e) => {
                ctx.out
                    .error(&format!("Could not render canonical YAML: {e}"), None);
