...
pub fn run(ctx: &mut Context) -> i32 {
    let (inputs, outputs) = match midi_io::list_ports() {
        ...
        Err(e) => {
            ctx.out
                .error(&format!("Could not enumerate MIDI ports: {e}"), None);
    ...
    ctx.out
        .info(&format!("MIDI input ports ({}):", inputs.len()), None);
    for p in &inputs {
        let marker = if is_ml10x(&p.name) {
            "   <-- ML10X"
        } else {
            ""
        };
    ...
    ctx.out.info("", None);
    ctx.out
        .info(&format!("MIDI output ports ({}):", outputs.len()), None);
    for p in &outputs {
        let marker = if is_ml10x(&p.name) {
            "   <-- ML10X"
        } else {
    ...
    let ml10x_in: Vec<&String> = inputs
        .iter()
        .filter(|p| is_ml10x(&p.name))
        .map(|p| &p.name)
        .collect();
    let ml10x_out: Vec<&String> = outputs
        .iter()
        .filter(|p| is_ml10x(&p.name))
        .map(|p| &p.name)
        .collect();
    ...
    if let (Some(i), Some(o)) = (ml10x_in.first(), ml10x_out.first()) {
        ctx.out.info(
            &format!("Found ML10X: input \"{i}\" + output \"{o}\"."),
            None,
        );
    }
