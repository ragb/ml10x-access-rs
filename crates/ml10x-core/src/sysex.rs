...
pub enum SysexError {
    ...
    #[error(
        "Segment at offset {offset} declares length {declared} but only {available} bytes are available"
    )]
    SegmentTruncated {
        offset: usize,
        declared: usize,
...
pub fn frame_with(
    ...
    let mut body: Vec<u8> =
        Vec::with_capacity(header.len() + segments.iter().map(|s| s.len()).sum::<usize>() + 2);
...
mod tests {
    ...
    fn frame_round_trip_with_segments() {
        ...
        let segs = vec![
            encode_segment(1, &[0x10, 0x20, 0x30]),
            encode_segment(2, b"hi"),
        ];
        ...
        assert_eq!(
            extracted,
            vec![(1u8, vec![0x10, 0x20, 0x30]), (2, b"hi".to_vec())]
        );
    }
