...
fn out_full(cap: &Value) -> Vec<Value> {
    cap["out_full"].as_array().expect("'out_full' array").clone()
}
...
fn every_outbound_handshake_message_parses() {
    ...
    for m in &out {
        let b = msg_bytes(m);
        let h = parse_header_with(ml10x::device::ML10X, &b, false).unwrap();
...
fn every_inbound_message_parses_with_length() {
    ...
    for m in in_full(&cap) {
        let b = msg_bytes(&m);
        let h = parse_header_with(ml10x::device::ML10X, &b, true).unwrap_or_else(|e| {
            panic!("inbound message rejected: {e}\nbytes: {b:?}")
        });
...
fn controller_uuid_decodes() {
    ...
    let info = in_full(&cap)
        .iter()
        .find(|m| msg_bytes(m).get(HeaderPos::FunctionId1 as usize) == Some(&(InboundClass::ControllerInfo as u8)))
        .map(msg_bytes)
...
fn preset_data_carries_preset_name_segment() {
    ...
    let name_seg = segs.iter().find(|(id, _)| *id == segment_id::PRESET_NAME).expect("name seg");
...
fn decode_preset_2_bypass_toggle_is_single_hop_delta() {
    ...
    let baseline = msg_bytes(out.iter().find(|m| m["phase"] == "baseline_before_bypass").unwrap());
    let after = msg_bytes(out.iter().find(|m| m["phase"] == "after_bypass_toggle").unwrap());
...
fn advanced_write_uses_p2_equal_2_and_omits_simple_segment() {
    ...
    let out = cap["out"].as_array().unwrap();
    let simple = msg_bytes(out.iter().find(|m| m["phase"] == "save_preset_2_simple").unwrap());
    let advanced = msg_bytes(out.iter().find(|m| m["phase"] == "save_preset_2_advanced").unwrap());
    ...
    let segs: std::collections::HashMap<u8, Vec<u8>> = iter_segments(&advanced).unwrap().into_iter().collect();
...
fn save_preset_name_segment_strips_trailing_spaces() {
    ...
    let segs: std::collections::HashMap<u8, Vec<u8>> = iter_segments(&b).unwrap().into_iter().collect();
